...
-- Eager stub: makes `:Octo` resolvable before the deferred preload runs.
-- `setup_octo()` recreates `:Octo` with octo's real (completing) command, so
-- we hand off by re-dispatching. Scheduled so windows open after startup.
